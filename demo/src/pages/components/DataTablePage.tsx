...
const dataTableProps: PropDef[] = [
  ...
  { name: 'columns', type: 'ColumnDef<T>[]', required: true, description: 'Column definitions array.' },
  ...
  { name: 'paginated', type: 'boolean', description: 'Enable pagination with page controls.' },
  { name: 'pageSize', type: 'number', description: 'Number of rows per page.' },
  ...
  { name: 'resizable', type: 'boolean', description: 'Allow resizing columns.' },
  { name: 'reorderable', type: 'boolean', description: 'Allow reordering columns.' },
  { name: 'exportable', type: 'boolean', description: 'Show export button (CSV/JSON).' },
  { name: 'virtualScroll', type: 'boolean', description: 'Enable virtual scrolling for large datasets.' },
  { name: 'stickyHeader', type: 'boolean', description: 'Make the table header sticky when scrolling.' },
  { name: 'sortBy', type: "{ column: string; direction: 'asc' | 'desc' }[]", description: 'Controlled sort state.' },
  { name: 'onSort', type: "(sort: { column: string; direction: 'asc' | 'desc' }[]) => void", description: 'Sort change callback.' },
  ...
  { name: 'error', type: 'ReactNode', description: 'Error message displayed on error state.' },
  ...
  { name: 'compact', type: 'boolean', description: 'Reduce cell padding for dense data.' },
  { name: 'bordered', type: 'boolean', description: 'Show cell borders.' },
  { name: 'responsiveMode', type: "'scroll' | 'card'", description: 'Responsive layout: scroll horizontally or stack as cards on mobile.' },
  ...
  { name: 'rowHeight', type: 'number', description: 'Row height in pixels (used by virtual scroll).' },
  { name: 'filterable', type: 'boolean', description: 'Enable per-column filter dropdowns.' },
  { name: 'filters', type: "Record<string, { value: string; operator: string }>", description: 'Controlled filter state.' },
  { name: 'onFilterChange', type: "(filters: Record<string, { value: string; operator: string }>) => void", description: 'Callback when filters change.' },
  { name: 'groupBy', type: 'string', description: 'Column ID to group rows by.' },
  ...
  { name: 'onGroupToggle', type: '(groupKey: string) => void', description: 'Callback when a group is expanded or collapsed.' },
  { name: 'editable', type: 'boolean', description: 'Enable cell editing.' },
  { name: 'onCellEdit', type: '(rowIndex: number, columnId: string, value: unknown) => void', description: 'Callback when a cell value is edited.' },
  { name: 'serverSide', type: 'boolean', description: 'Enable server-side mode. Disables client-side sort/filter/page.' },
  { name: 'totalRows', type: 'number', description: 'Total row count for server-side pagination.' },
  { name: 'onFetchData', type: '(params: { page: number; pageSize: number; sortBy: { column: string; direction: \'asc\' | \'desc\' }[]; filters: Record<string, { value: string; operator: string }>; search: string }) => void', description: 'Callback to fetch data from server with sort, filter, page params.' },
  { name: 'autoSizeColumns', type: 'boolean', description: 'Auto-size columns to fit content width.' },
  { name: 'showSuggestions', type: 'boolean', description: 'Show search suggestions.' },
]
