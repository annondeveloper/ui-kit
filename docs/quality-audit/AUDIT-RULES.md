...
### 8. PROP/TIER DRIFT CHECK (Score: /10)

The single source of truth for props and tiers is `dist/component-meta.json`, auto-generated by `scripts/extract-component-meta.ts` from TypeScript interfaces. Every consumer must match it.

- [ ] Props in demo page `PropDef[]` match `component-meta.json` exactly
- [ ] Props in MCP registry match `component-meta.json` exactly
- [ ] Tier array in demo page tier cards matches `component-meta.json`
- [ ] Tier array in MCP registry matches `component-meta.json`
- [ ] Import paths in code generators match `component-meta.json`
- [ ] Import paths in demo tier cards match `component-meta.json`
- [ ] Code generator prop defaults match actual defaults from source
- [ ] No hardcoded props that aren't in the TypeScript interface
- [ ] New props added to interface automatically appear in meta
- [ ] Run `npm run build:meta` and diff — no unexpected changes

**How to check:**
```bash
npm run build:meta
# Then compare component-meta.json props against demo page PropDef arrays
# and MCP registry entries. Any mismatch = drift = bug.
```
