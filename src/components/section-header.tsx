...
export interface SectionHeaderProps extends HTMLAttributes<HTMLElement> {
  ...
  description?: string
  /** Action slot rendered on the right side, e.g. <Button variant="ghost">View All</Button> */
  action?: ReactNode
...
const sectionHeaderStyles = css`
  @layer components {
    @scope (.ui-section-header) {
      ...
      /* Size variants */
      :scope[data-size="sm"] .ui-section-header__title {
        font-size: clamp(0.875rem, 0.8rem + 0.25vw, 1rem);
      }
      :scope[data-size="md"] .ui-section-header__title {
        font-size: clamp(1.125rem, 1rem + 0.5vw, 1.375rem);
      }
      :scope[data-size="lg"] .ui-section-header__title {
...
/**
 * SectionHeader — section title with optional description and action slot.
 * Renders as a flex row with title/description on the left and an action on the right.
 */
export const SectionHeader = forwardRef<HTMLElement, SectionHeaderProps>(
  (
    ...
    const cls = useStyles('section-header', sectionHeaderStyles)
